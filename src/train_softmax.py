...
def get_symbol(args, arg_params, aux_params):
    ...
    if args.loss_type == 0:  # softmax
        ...
        s = args.margin_s
        assert s > 0.0
        _weight = sym.L2Normalization(_weight, mode='instance')
        nembedding = sym.L2Normalization(embedding, mode='instance', name='fcin_fgg') * s
        fc7 = sym.FullyConnected(data=nembedding, weight=_weight, no_bias=True, num_hidden=args.num_classes,
                                 name='fc7_fgg')
        # sftmaxg = sym.SoftmaxOutput(data=fc7, labe=gt_label, name='softmax', normalization='valid')
        sftmax = mx.symbol.softmax(data=sym.BlockGrad(fc7), name='softmax_fgg')
        _label = mx.sym.one_hot(gt_label, depth=args.num_classes, on_value=-1.0, off_value=0.0, name='onehot_label')
        sftmaxg = sftmax - _label
        
        print('!!! ',
              sftmaxg.infer_shape(data=(args.per_batch_size, 3, 112, 112,),
                                  softmax_label=(args.per_batch_size,))[1]
              )
        
        def get_logitg(ymodel, ones, shape=args.num_classes):
            mat = sym.linalg_gemm2(
                sym.reshape(ymodel, (-1, shape, 1)),
                sym.reshape(ymodel, (-1, 1, shape)), alpha=1
            )
            ones = sym.reshape(ones, (-1, shape, 1))
            mat = -sym.linalg_gemm2(mat, ones, alpha=1)
            mat = sym.reshape(mat, (-1, shape,))
            
            ones = sym.reshape(ones, (-1, shape))
            xgradm = sym.broadcast_add(
                mat,
                sym.elemwise_mul(ymodel, ones)
            )
            return xgradm
        
        logitg = get_logitg(sftmax, sftmaxg)
        
        print('!!! ',
              logitg.infer_shape(data=(args.per_batch_size, 3, 112, 112,),
                                 softmax_label=(args.per_batch_size,))[1]
              )
        
        def get_feang(wei, ones, ):
            xgradm = sym.linalg_gemm2(ones, wei, alpha=1, )
            return xgradm
        
        feang = get_feang(_weight, logitg)
        
        print('!!! ',
              feang.infer_shape(data=(args.per_batch_size, 3, 112, 112,),
                                softmax_label=(args.per_batch_size,))[1],
              )
        
        def get_feag(xm, ym, ones, ):
            sigma = sym.sum(sym.elemwise_mul(ones, ym, ), axis=1)
            sigma = sym.reshape(sigma, (-1, 1))
            normx = sym.sqrt(sym.sum(sym.elemwise_mul(xm, xm), axis=1))
            normx = sym.reshape(normx, (-1, 1))
            xgrad = ones - sym.broadcast_mul(sigma, ym)
            xgrad = sym.broadcast_div(xgrad, normx)
            return xgrad
        
        feag = get_feag(embedding, nembedding, feang)
        print('!!! feag',
              feag.infer_shape(data=(args.per_batch_size, 3, 112, 112,),
                               softmax_label=(args.per_batch_size,))[1]
              )
        # feag = feang
        feag = sym.BlockGrad(feag)
        embedding = embedding + 0.1 * feag
        nembedding = sym.L2Normalization(embedding, mode='instance', name='fcin') * s
    ...
    if args.ce_loss:
        ...
        # body = mx.symbol.softmax(data=fc7) #todo ??
